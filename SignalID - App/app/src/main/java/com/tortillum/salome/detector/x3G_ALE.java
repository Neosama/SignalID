...
// 3G ALE (ARCS) is defined by STANAG 4538 and MIL-STD-188-141B Appendix C
...
public class x3G_ALE {
    private int MARGIN_ERROR = 5; // in Hertz

    private int frequencie_A = 2105;
    private int frequencie_B = 1875;
    ...
    private int score = 0; // MAX = 230 (fq_A - fq_B without margin)

    public x3G_ALE(int sampleRate, int n_fft, Pair[] listFqs) {
        for (int i = 0; i < 64; i++) {
        ...
        System.out.println("[DBG - x3G_ALE.java] score : " + score);
    }

    public boolean is3G_ALE() {
        if(find_frequencie_A && find_frequencie_B && score <= 25 && score >= 10)
            return true;
        return false;
    }
}
