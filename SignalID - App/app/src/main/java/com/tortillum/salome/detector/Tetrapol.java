...
//listFqs : TOP 32 for fq_A/fq_B & TOP 64 between
...
public class Tetrapol {
	...
	private int frequencie_A = 2049;
	private int frequencie_B = 950;
	...
	private int score = 0; // MAX = 1099 (fq_A - fq_B without margin)

	public Tetrapol(int sampleRate, int n_fft, Pair[] listFqs) {
		for(int i = 0; i < 64; i++) {
		...
		System.out.println("[DBG - Tetrapol.java] score_TFC : " + score);
	}
	
	public boolean isTetrapol() {
		if(find_frequencie_A && find_frequencie_B && score <= 1135 && score >= 1110)
			return true;
		return false;
	}
}
