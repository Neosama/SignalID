...
import com.tortillum.salome.signal.FT4;
...
public class T_SFFT {
	...
	public T_SFFT(String pPath, Score[] pScores, Duration[] pDurations) throws IOException {
		...
		for(int i = 0; i < allData.length/windowStep; i++) {
			if(totalWS+(allData.length/windowStep) < allData.length) {
				...
				// FT8 (with duration check)
				new FT8(44100, n_sfft, frequenciesArray_sfft, pDurations);
				...
				// FT4 (with duration check)
				new FT4(44100, n_sfft, frequenciesArray_sfft, pDurations);
